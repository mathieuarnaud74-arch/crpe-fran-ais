...
import { DashboardSessionProgress, MathDomainKey, LearningStatus } from "@/types/domain";

type Params = Promise<{ domain: string }>;

function isMathDomainKey(value: string): value is MathDomainKey {
  return MATH_DOMAIN_ORDER.includes(value as MathDomainKey);
}

export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  const { domain } = await params;
  if (!isMathDomainKey(domain)) {
    return { title: "Domaine introuvable" };
  }
  const config = MATH_DOMAIN_CONFIG[domain];
  return {
    title: config.label,
...
export default async function MathDomainPage({
  params,
  searchParams,
}: {
  params: Params;
  ...
  const data = await getDashboardData(user.id, premium, "Mathematiques");
  const config = MATH_DOMAIN_CONFIG[domain];
  const pathname = `/maths/${domain}`;
      ...
      <Panel className="border-border bg-card">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div className="space-y-3">
            <Badge tone="accent">Mathématiques</Badge>
            <div>
