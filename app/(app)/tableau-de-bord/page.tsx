  ...
  return (
    <div className="relative h-11 w-11 shrink-0">
      <svg viewBox="0 0 44 44" className="h-11 w-11 -rotate-90" aria-hidden="true">
        <circle cx="22" cy="22" r={radius} fill="none" stroke="rgba(245,241,232,0.12)" strokeWidth="3.5" />
        <circle
...
export default async function DashboardPage() {
  ...
  const [data, mathData, diagnostic] = await Promise.all([
  ...
  const buckets: { a_revoir: typeof data.sessionProgress; non_commencee: typeof data.sessionProgress; en_cours: typeof data.sessionProgress } = {
    a_revoir: [], non_commencee: [], en_cours: [],
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          {data.totalAttempts > 0 && (
            <div className="flex items-center gap-3 rounded-[1.25rem] border border-paper/10 bg-paper/[0.07] px-4 py-3 backdrop-blur-sm">
              <DailyGoalRing current={Math.min(data.attemptsToday, 15)} goal={15} />
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-paper/50">Objectif du jour</p>
                <p className="text-sm font-semibold text-paper">{Math.min(data.attemptsToday, 15)}/15 questions</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-3">
          <div data-tour="stats-strip" className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-4">
            <div className="bg-card px-5 py-4 text-center">
              <p className="font-serif text-3xl font-semibold text-pine"><AnimatedCounter end={data.masteredSessions} /></p>
              <p className="mt-1 text-xs font-medium tracking-wide text-muted">Maîtrisées</p>
              <p className="mt-0.5 text-xs text-muted">sur {data.totalSeries} série(s)</p>
            </div>
            <div className="bg-card px-5 py-4 text-center">
              ...
              <p className="mt-0.5 text-xs text-muted"><AnimatedCounter end={data.totalAttempts} /> tentative(s)</p>
            </div>
            <div className="bg-card px-5 py-4 text-center">
              ...
              <p className="mt-0.5 text-xs text-muted"><AnimatedCounter end={data.completedSeries} /> terminée(s)</p>
            </div>
            <div className="bg-card px-5 py-4 text-center">
              ...
              {!premium && (
                <p className="mt-0.5 text-xs text-muted">sur {env.freeDailyQuestionLimit}/jour</p>
              )}
            </div>
          </div>
          ...
          <div>
            <div className="mb-1.5 flex justify-between text-xs text-muted">
              ...
              <span>{data.masteredSessions}/{data.totalSeries} maîtrisées ({masteryPercent}%)</span>
            </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-accentSecondary/25 shadow-subtle">
          <div className="border-b border-accentSecondary/20 bg-[linear-gradient(135deg,rgba(164,104,73,0.12),rgba(252,250,246,1)_60%)] px-4 py-3 sm:px-6 sm:py-4">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-accentSecondary">Session du jour</p>
                <h2 className="mt-1 font-serif text-2xl font-semibold text-ink">Plan du jour</h2>
              </div>
              <ButtonLink href="/exercices" variant="secondary">Catalogue complet</ButtonLink>
            </div>
          </div>
            ...
            {plan.length === 0 ? (
              <div className="rounded-xl border border-border bg-paper px-4 py-3">
                <p className="text-sm text-muted">
                  ...
                  <Link href="/francais" className="font-semibold text-ink underline underline-offset-2">page Français</Link>{" "}
      ...
      {/* ── Révision intelligente (SRS) — streams independently ── */}
      <Suspense fallback={
        <Panel>
        ...
        <DashboardSrsSection userId={user.id} />
      </Suspense>

      {/* ── Domaines Français ── */}
      <DomainDirectoryTable
        ...
        <div className="space-y-4">
          <div className="grid gap-4 2xl:grid-cols-2">
            <CollapsiblePanel
              ...
              defaultOpen={false}
            >
              <TopicFocusList items={data.priorityItems} emptyText="Aucune priorité forte détectée." />
            </CollapsiblePanel>
            <CollapsiblePanel
              title="Fragilités"
              tone="high"
              ...
              defaultOpen={false}
            >
              <TopicFocusList items={data.fragileItems} emptyText="Aucune fragilité notable." />
            </CollapsiblePanel>
          </div>
              ...
              <div className="grid gap-3 xl:grid-cols-2">
                {data.frequentMistakes.map((mistake) => (
                  <article key={mistake.key} className="rounded-xl border border-border/60 bg-paper px-5 py-3">
                    ...
                    <p className="mt-1 text-xs text-muted/70">{mistake.subdomainLabel} · {formatDate(mistake.lastAnsweredAt)}</p>
                  </article>
                ))}
              </div>
            )}
          ...
          <Panel className="relative overflow-hidden border-accent/20 bg-card">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <ButtonLink href="/progression" variant="secondary">Détail</ButtonLink>
            </div>
                  ...
                  .map((session) => (
                    ...
                    >
                      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-accent/10">
                        <svg className="h-3.5 w-3.5 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} aria-hidden="true">
        ...
        <div className="grid gap-4 2xl:grid-cols-2">
          <Panel data-tour="heatmap">
            <div className="flex items-center justify-between gap-4 border-b border-border pb-4">
              <div>
                <p className="text-[0.7rem] font-bold uppercase tracking-[0.18em] text-[#476257]">R&eacute;gularit&eacute;</p>
                <h2 className="mt-1 font-serif text-xl font-semibold text-ink">Activit&eacute; r&eacute;cente</h2>
              </div>
          ...
          {quickChallengeSession ? (
            <div className="relative overflow-hidden rounded-[1.75rem] border border-accentSecondary/20 bg-[linear-gradient(135deg,#476257_0%,#394E45_60%,#2C2420_100%)] px-5 py-5 shadow-panel sm:px-6 sm:py-6">
              <span aria-hidden className="pointer-events-none absolute -right-10 -top-10 h-48 w-48 rounded-full bg-paper/[0.04] select-none" />
              <span aria-hidden className="pointer-events-none absolute right-16 bottom-0 h-28 w-28 rounded-full bg-paper/[0.03] select-none" />
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <Badge className="border-paper/20 bg-paper/10 text-paper/70">{quickChallengeSession.subdomainLabel}</Badge>
                <Badge className="border-paper/20 bg-paper/10 text-paper/70">{quickChallengeSession.questionCount} questions</Badge>
              </div>
              ...
              {quickChallengeSession.answeredQuestions > 0 && (
                <div className="mt-3">
                  <div className="flex justify-between text-[10px] text-paper/50">
                    ...
                    {quickChallengeSession.correctRate !== null && <span>{quickChallengeSession.correctRate} %</span>}
                  </div>
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                ...
                <ButtonLink href="/exercices" variant="ghost" className="text-paper/70 hover:text-paper hover:bg-paper/10">
                  Catalogue
                </ButtonLink>
              </div>
              ...
              {otherSuggestions.length > 0 && (
                <div className="mt-4 border-t border-paper/10 pt-3">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-paper/40">Aussi sugg&eacute;r&eacute;</p>
                  <div className="mt-2 flex flex-col gap-1.5">
                    {otherSuggestions.map((s) => (
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-sm font-medium text-paper/80 group-hover:text-paper">{s.title}</p>
            ...
            <Panel className="flex flex-col items-center justify-center border-successBorder/30 bg-successBg/30 text-center">
              ...
              <p className="mt-3 font-serif text-xl font-semibold text-ink">Toutes les s&eacute;ries sont ma&icirc;tris&eacute;es !</p>
              <p className="mt-1 text-sm text-muted">De nouvelles s&eacute;ries seront bient&ocirc;t disponibles.</p>
            </Panel>
          )}
        </div>
      )}
        ...
        <Panel>
          <div className="flex items-center justify-between gap-4 border-b border-border pb-4">
            <div>
              ...
              <h2 className="mt-1 font-serif text-xl font-semibold text-ink">Derni&egrave;res r&eacute;ponses</h2>
            </div>
            ...
            <div className="mt-4 grid gap-2 xl:grid-cols-2">
              {data.recentActivity.slice(0, 4).map((activity) => (
                ...
                >
                  ...
                  <p className="mt-1.5 text-sm font-medium leading-6 text-ink">{activity.instruction}</p>
                  <p className="mt-1 text-xs text-muted/70">{activity.subdomainLabel} · {formatDate(activity.answeredAt)}</p>
                </article>
              ))}
            </div>
          )}
