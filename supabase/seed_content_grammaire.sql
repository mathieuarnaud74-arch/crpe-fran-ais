...
'{"mode":"single_choice","value":"b"}'::jsonb,
'Pour mettre en relief le SUJET, on utilise "c''est...qui" — c''est la réponse b) ! Pour mettre en relief un COD ou un CC, on utilise "c''est...que". Ici, on veut insister sur "elle" (le sujet) → "C''est elle qui a trouvé la solution." La réponse a) met en relief le COD "la solution" avec "c''est...que" (et note l''accord du participe). La réponse c) est agrammaticale. Mémorise : sujet → "qui", tout le reste → "que".',
'valide','CRPE Français V2','free',true),
...
'{"mode":"single_choice","value":"b"}'::jsonb,
'Pour mettre en relief un COD, c''est "c''est...que" — la réponse b) ! "Qui" est réservé à la mise en relief du SUJET. Ici "ce rapport" est COD → "c''est ce rapport QU''elle a rédigé". Et note l''accord du participe : "ce rapport" est le COD placé avant "rédigé" → accord → "rédigé" reste masculin singulier. La réponse a) met en relief le sujet "elle" avec "qui". La réponse c) est agrammaticale. Sujet → qui. Tout le reste → que.',
'valide','CRPE Français V2','free',true),
