...
'[{"id":"a","label":"La capacité à identifier la signification des mots à l''oral"},{"id":"b","label":"La capacité à identifier et manipuler les unités sonores de la langue (syllabes, phonèmes) indépendamment du sens"},{"id":"c","label":"La connaissance de l''alphabet et des lettres de l''écrit"}]'::jsonb,
...
'[{"id":"a","label":"Le fait que les lettres de l''alphabet ont un ordre fixe"},{"id":"b","label":"La correspondance systématique entre graphèmes (lettres ou groupes de lettres) et phonèmes (sons)"},{"id":"c","label":"La règle selon laquelle toutes les langues utilisent les mêmes lettres"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"La plus petite unité de sens d''un mot (ex : préfixe, suffixe)"},{"id":"b","label":"La plus petite unité sonore distinctive de la langue orale"},{"id":"c","label":"Un groupe de lettres correspondant à un son long"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Copier des mots au tableau"},{"id":"b","label":"Identifier combien de sons on entend dans le mot « chat »"},{"id":"c","label":"Réciter l''alphabet de A à Z"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"La capacité à lire un texte à voix haute avec exactitude, vitesse et prosodie appropriée"},{"id":"b","label":"La capacité à comprendre les inférences d''un texte"},{"id":"c","label":"La vitesse à laquelle un élève mémorise le vocabulaire nouveau"}]'::jsonb,
...
'[{"id":"a","label":"Il n''y a aucune différence : un graphème est toujours une seule lettre"},{"id":"b","label":"Un graphème peut être composé d''une ou plusieurs lettres correspondant à un phonème (ex : « ou », « eau », « ch »)"},{"id":"c","label":"Un graphème est une lettre majuscule, une lettre est une minuscule"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Cycle 1 (maternelle), dès la petite section"},{"id":"b","label":"Cycle 2 (CP-CE1-CE2), particulièrement au CP"},{"id":"c","label":"Cycle 3 (CM1-CM2-6ème), une fois la lecture autonome acquise"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Erreur lexicale : il ne connaît pas la graphie du verbe « jouer »"},{"id":"b","label":"Erreur de morphologie verbale : confusion entre l''infinitif (-er) et le participe passé (-é) après l''auxiliaire"},{"id":"c","label":"Erreur phonétique : le son /e/ final peut s''écrire de plusieurs façons"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
'Après l''auxiliaire "avoir", on emploie le participe passé : "a joué". L''élève a utilisé l''infinitif "jouer" à la place. C''est une erreur de morphologie verbale grammaticale : l''élève confond les deux formes, phonétiquement identiques (/ʒue/). La piste phonétique (réponse c) est une explication partielle de la confusion, mais l''erreur relève bien de la morphologie verbale.',
'valide','CRPE Français V2 — Didactique','free',true),
...
'[{"id":"a","label":"Erreur d''accord sujet-verbe : le verbe n''est pas accordé avec le sujet pluriel « les garçons »"},{"id":"b","label":"Erreur lexicale : le verbe « jouer » est mal orthographié"},{"id":"c","label":"Erreur de conjugaison : le temps verbal est incorrect"}]'::jsonb,
'{"mode":"single_choice","value":"a"}'::jsonb,
...
'[{"id":"a","label":"La copie répétée du mot correct (écrire 10 fois le mot)"},{"id":"b","label":"L''encodage actif : regarder le mot, le cacher, l''écrire de mémoire, puis vérifier (méthode « voir-cacher-écrire-vérifier »)"},{"id":"c","label":"Souligner en rouge les mots mal orthographiés dans les copies"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"L''hypercorrection"},{"id":"b","label":"L''écriture phonétique ou phonétisation"},{"id":"c","label":"La surgénéralisation morphologique"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Lui faire recopier 20 fois la règle d''utilisation de « à »"},{"id":"b","label":"Lui apprendre à substituer « avait » : si la substitution est possible, c''est le verbe « a » ; sinon, c''est la préposition « à »"},{"id":"c","label":"Lui interdire d''utiliser le mot « à » dans ses rédactions jusqu''à ce qu''il maîtrise la règle"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Remplacer le verbe par un verbe du 3e groupe : si on peut dire « il a mordre », c''est -er ; sinon c''est -é"},{"id":"b","label":"Remplacer le verbe par « vendre » ou « prendre » : si on peut dire « il a vendu/pris », alors c''est un participe passé en -é"},{"id":"c","label":"Compter les syllabes du verbe : les verbes de plus de deux syllabes prennent toujours -er"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"La démarche déductive part de la règle vers les exemples ; la démarche inductive part des exemples vers la règle"},{"id":"b","label":"La démarche déductive est réservée au cycle 3 ; la démarche inductive est réservée au cycle 2"},{"id":"c","label":"La démarche déductive utilise des textes littéraires ; la démarche inductive utilise des exercices systématiques"}]'::jsonb,
'{"mode":"single_choice","value":"a"}'::jsonb,
...
'[{"id":"a","label":"Une activité manuelle où les élèves déplacent des étiquettes de mots"},{"id":"b","label":"Une opération effectuée sur un énoncé (déplacement, suppression, substitution, addition) pour en révéler les propriétés grammaticales"},{"id":"c","label":"Une technique de mémorisation des terminaisons verbales par gestes associés"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Vérifier que les élèves ont appris les définitions par cœur"},{"id":"b","label":"Permettre aux élèves de construire des catégories grammaticales par observation de régularités et de différences"},{"id":"c","label":"Gagner du temps en évitant les explications orales"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Le déplacement en tête de phrase"},{"id":"b","label":"La pronominalisation : remplacer le groupe par un pronom sujet (il, elle, ils, elles, on)"},{"id":"c","label":"La suppression : enlever le groupe et vérifier que la phrase reste grammaticalement correcte"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"La grammaire que l''élève a apprise mais qu''il a oubliée"},{"id":"b","label":"La connaissance intuitive de la langue que l''élève possède du fait de son usage quotidien, sans en avoir conscience"},{"id":"c","label":"La grammaire normative non enseignée dans les programmes officiels"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Cycle 1 (grande section de maternelle)"},{"id":"b","label":"Cycle 2 (CP-CE1-CE2)"},{"id":"c","label":"Cycle 3 (CM1-CM2-6ème)"}]'::jsonb,
...
'[{"id":"a","label":"Permettre aux élèves de mémoriser des phrases types à réutiliser"},{"id":"b","label":"Servir de base d''observation pour que les élèves induisent eux-mêmes une règle ou une régularité grammaticale"},{"id":"c","label":"Illustrer la règle que l''enseignant vient d''expliquer"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Du décodage multiplié par la compréhension orale"},{"id":"b","label":"De la vitesse de lecture multipliée par la mémoire de travail"},{"id":"c","label":"De la richesse du vocabulaire multipliée par la motivation à lire"}]'::jsonb,
'{"mode":"single_choice","value":"a"}'::jsonb,
...
'[{"id":"a","label":"Une information explicitement formulée dans le texte que le lecteur doit retrouver"},{"id":"b","label":"Une information implicite que le lecteur doit construire en combinant des éléments du texte et ses connaissances"},{"id":"c","label":"Un résumé que le lecteur produit après avoir lu le texte"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Lire le texte en entier sans s''arrêter, puis répondre aux questions"},{"id":"b","label":"Se poser des questions pendant la lecture (« Qu''est-ce que je comprends jusqu''ici ? »), faire des prédictions et vérifier"},{"id":"c","label":"Surligner tous les mots de liaison du texte"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Elle développe uniquement la fluence de lecture des élèves"},{"id":"b","label":"Elle permet de travailler la compréhension de textes complexes sans que le décodage soit une barrière, en enrichissant le vocabulaire et les connaissances"},{"id":"c","label":"Elle remplace avantageusement toutes les autres pratiques de lecture"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Pour vérifier que les élèves ont bien fait leurs devoirs"},{"id":"b","label":"Pour orienter leur attention et faciliter la construction d''inférences pendant la lecture"},{"id":"c","label":"Pour éviter qu''ils lisent le texte trop vite"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"La représentation mentale de la situation décrite par le texte, construite par le lecteur à partir du texte et de ses connaissances"},{"id":"b","label":"Le schéma narratif du texte (situation initiale, péripétie, situation finale)"},{"id":"c","label":"Le plan de lecture que l''enseignant établit avant de présenter un texte aux élèves"}]'::jsonb,
'{"mode":"single_choice","value":"a"}'::jsonb,
...
'[{"id":"a","label":"Le vocabulaire passif est le vocabulaire des textes écrits, le vocabulaire actif est le vocabulaire de l''oral"},{"id":"b","label":"Le vocabulaire passif est compris mais non utilisé spontanément ; le vocabulaire actif est maîtrisé et mobilisé en production"},{"id":"c","label":"Le vocabulaire passif est peu courant, le vocabulaire actif est fréquent dans la langue"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Un mot dont le sens est plus spécifique que celui d''un autre mot (ex : « caniche » par rapport à « chien »)"},{"id":"b","label":"Un terme générique qui inclut dans son sens plusieurs termes plus spécifiques (ex : « animal » par rapport à « chien », « chat », « oiseau »)"},{"id":"c","label":"Un mot de sens contraire à un autre mot (ex : « chaud » et « froid »)"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Une syllabe d''un mot"},{"id":"b","label":"La plus petite unité de sens : racine, préfixe ou suffixe"},{"id":"c","label":"Un son distinct qui modifie le sens d''un mot"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Le champ lexical regroupe les mots liés à un même thème ou domaine ; le champ sémantique regroupe les différents sens d''un même mot"},{"id":"b","label":"Le champ lexical concerne l''écrit, le champ sémantique concerne l''oral"},{"id":"c","label":"Il n''y a pas de différence : les deux termes sont synonymes"}]'::jsonb,
'{"mode":"single_choice","value":"a"}'::jsonb,
...
'[{"id":"a","label":"Faire copier la définition du mot dans le cahier de vocabulaire"},{"id":"b","label":"Demander aux élèves d''utiliser le mot dans une phrase originale, dans un contexte différent de celui d''apprentissage"},{"id":"c","label":"Afficher le mot au tableau pendant toute la semaine"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
...
'[{"id":"a","label":"Le fait qu''un mot ait plusieurs orthographes acceptées"},{"id":"b","label":"Le fait qu''un même mot ait plusieurs sens différents selon les contextes"},{"id":"c","label":"Le fait que deux mots différents aient le même sens (synonymie)"}]'::jsonb,
'{"mode":"single_choice","value":"b"}'::jsonb,
