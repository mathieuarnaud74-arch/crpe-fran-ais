...
INSERT INTO public.exercises (
  ...
  'Didactique de la géométrie plane — Enseigner les figures', 'Intermediaire',
  'vrai_faux',
  'Un élève de CM1 trace un losange au compas en procédant ainsi : il trace un segment [AB], puis deux arcs de cercle de rayon AB centrés en A et B qui se coupent en C et D, et relie A-C-B-D. Sa construction est valide car elle garantit quatre côtés de même longueur.',
  NULL,
  NULL,
  '{"mode":"single","value":"vrai"}'::jsonb,
  'La construction est valide. En traçant deux arcs de cercle de rayon AB (noté r = AB) centrés en A et en B, les points d''intersection C et D vérifient : AC = BC = AD = BD = r = AB. On obtient donc le quadrilatère ACBD (en reliant les points dans l''ordre) qui a quatre côtés de même longueur : c''est bien un losange. Cette construction au compas est une application de la géométrie instrumentée : l''élève utilise le compas pour reporter des longueurs et construire une figure ayant la propriété requise (quatre côtés égaux). Analyser les constructions des élèves est une compétence clé pour le CRPE : il faut vérifier si la procédure garantit mathématiquement les propriétés attendues, et non simplement si le résultat « ressemble » à la figure souhaitée.',
